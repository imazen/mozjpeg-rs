...
mod tests {
    ...
    ///
    /// Rust generates exactly 64 scans for YCbCr, matching C mozjpeg's
    ...
    ///
    /// Migrated from tests/scan_verification.rs so scan_optimize can be pub(crate).
    #[test]
    fn test_scan_generation_structure_vs_c_reference() {
        ...
        for (i, (rust, c)) in rust_scans.iter().zip(c_scans.iter()).enumerate() {
            ...
            assert_eq!(
                ...
                "Scan {i}: comps_in_scan mismatch (Rust={}, C={c_comps})",
                rust.comps_in_scan
            );
            assert_eq!(rust.ss, *c_ss, "Scan {i}: Ss mismatch");
            assert_eq!(rust.se, *c_se, "Scan {i}: Se mismatch");
            assert_eq!(rust.ah, *c_ah, "Scan {i}: Ah mismatch");
            assert_eq!(rust.al, *c_al, "Scan {i}: Al mismatch");

            // Verify component indices
            for j in 0..(*c_comps as usize) {
                assert_eq!(
                    rust.component_index[j], c_indices[j],
                    "Scan {i}: component_index[{j}] mismatch"
                );
            }
        }
        ...
        // 7 refinement scans total (3 luma Ah=1,2,3 + 4 chroma: Cb/Cr × Ah=1,2)
        ...
        // Full 1-63 scans at Al=0 (luma at index 12, chroma at 42, 43)
