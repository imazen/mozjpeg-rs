...
impl zencodec::encode::EncoderConfig for MozjpegEncoderConfig {
    ...
    /// - `ApproxButteraugli(d)` → coarse linear map onto the quality dial.
    ///
    /// **No measured table yet.** The zenmetrics omni fleet sweep cannot seed
    /// these maps: its `zenjpeg` cells drive `with_generic_quality`, which for
    /// zenjpeg targets a **zensim Profile A** score (a perceptual target), not
    /// the raw IJG quality dial — so its `q` axis is not a JPEG quality and does
    /// not transfer to mozjpeg's raw `quality()`. A raw-JPEG-quality sweep
    /// (vary `quality`, measure butteraugli max-norm / SSIM2) is needed before
    /// these arms can become measured tables. Until then they stay linear.
    ///
...
mod tests {
    ...
    #[test]
    fn supported_descriptors_include_rgba() {
