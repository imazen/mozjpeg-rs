...
- vCPU-aware encode resource estimation via zencodec's unified `estimate` API:
  ...
  default, returning a core-adjusted `ResourceEstimate`. The estimate is an
  uncalibrated structural placeholder (serial encode; peak ≈ input + libjpeg
  working buffers), not a measured model.
- Versioned public-API surface snapshot at `docs/public-api/mozjpeg-rs.txt`,
