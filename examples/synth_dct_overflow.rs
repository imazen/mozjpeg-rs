...
// x86_64-only: uses `SimdOps::avx2_i16()` and `dct::avx2_archmage`, both of
// which exist only on x86_64. Gated so `cargo test` / `cargo build
// --all-targets` compiles on aarch64.
...
fn main() {
    eprintln!("x86_64-only DCT overflow probe; nothing to do on this target");
}
