...
// This example is an x86_64 AVX2 debug tool — it uses core::arch::x86_64
// intrinsics, `is_x86_feature_detected!`, and #[target_feature(enable = "avx2")]
// throughout. Without an arch gate, `cargo test` / `cargo build --all-targets`
// fails to COMPILE on aarch64, which is how it stood before 2026-07-28: the
// test suite could not be built on ARM at all.
...
fn main() {
    eprintln!("x86_64-only AVX2 debug tool; nothing to do on this target");
}
